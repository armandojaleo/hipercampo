...
| `hc_recall(query, k, include_history, max_scan, nav, nav_auto)` | Recupera por similitud. Puede **abstenerse** (devolver `[]`). `max_scan` acota CPU/RAM para uso embebido/robots; `nav_auto` deja que el núcleo use navegación si es seguro. |
...
recupera cualquier campo por *unbinding* (`hipercampo/cycle/roles.py`):
...
- **Seguridad**: el texto recuperado es **dato, no instrucciones**. Salvaguardas
  integradas (`hipercampo/support/safety.py`): `hc_remember` avisa de posibles **secretos**
