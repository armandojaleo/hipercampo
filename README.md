...
| `hc_recall(query, k, include_history, max_scan, nav, nav_auto)` | Retrieve by similarity. Can **abstain** (return `[]`). `max_scan` bounds CPU/RAM for embedded/robot use; `nav_auto` lets the core choose graph navigation when safe. |
