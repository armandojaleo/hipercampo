...
def test_health_prueba_la_escritura_de_verdad():
    ...
    assert h.get("check") == "quick_check", "por defecto debe ser barato"
    ...
    assert "writes_since_sleep" in h and "wal_bytes" in h, h


def test_health_completo_bajo_demanda():
    hc = memory("est_health_full")
...
def test_solo_se_reintenta_lo_transitorio():
    ...
    assert _is_transient(sqlite3.ProgrammingError("Cannot operate on a closed database."))
    assert not _is_transient(sqlite3.OperationalError(
        "attempt to write a readonly database"))
    assert not _is_transient(sqlite3.DatabaseError("database disk image is malformed"))
    assert not _is_transient(sqlite3.OperationalError("no such column: namespace"))
