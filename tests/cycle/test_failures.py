...
def test_read_only_database_reports_without_retrying():
    ...
    try:
        ...
        assert r.get("retried") is False, (
            f"retried a write on a read-only DB: {r}")
...
def test_full_disk_reports_and_does_not_corrupt():
    ...
    assert isinstance(r, dict) and "error" in r, f"debió avisar: {r}"
    assert r.get("retried") is False, f"retried with a full disk: {r}"

    hc.store.add = original                         # el disco "se vacía"
    assert hc.health()["healthy"] is True, "el fallo de espacio no debe corromper"
...
def test_killed_writer_process_does_not_corrupt():
    ...
    assert h["integrity"] == "ok", f"la muerte súbita corrompió la BD: {h}"
    total = hc.stats()["total_physical"]
...
def test_killed_sleep_process_does_not_corrupt_and_is_retried():
    ...
    h = hc.health(full=True)
    assert h["integrity"] == "ok", f"morir durmiendo corrompió la BD: {h}"
    ...
    r = hc.sleep()                                  # retry the sleep, now at peace
