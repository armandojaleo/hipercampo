...
HERRAMIENTAS = {
    ...
    # `tipo` -> `kind` al pasar la superficie pública a inglés. Es un cambio de
    # firma DELIBERADO, y este test lo cazó: por eso se toca aquí a mano y no se
    # relaja la comprobación. `Hipercampo.learn()` sigue aceptando `tipo=` y los
    # valores en español, así que quien ya llamaba a la API de Python no se rompe;
    # lo que cambia es el nombre que ve el LLM en la herramienta MCP.
    "hc_learn":         {"text": False, "kind": True},
    "hc_identity":      {"k": True},
...
def test_forma_de_stats_y_health():
    ...
    assert {"links", "knn", "dream_pending", "knn_coverage"} <= set(s["graph"]), s
    assert {"items", "active"} <= set(s["identity"]), s
    h = hc.health()
    assert {"db", "namespace", "integrity", "schema", "readable",
...
def test_forma_de_error_resiliente():
    ...
    assert {"error", "suggestion"} <= set(r), r
    hc.store.path = ""
